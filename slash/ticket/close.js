...
const { Permissions, MessageButton, MessageActionRow, MessageEmbed } = discord
...
module.exports = {
	...
	async execute(client, interaction, args) {
		...
		const result = await ticketSchema.findOne({ serverId: interaction.guild.id, ticketChannel: interaction.channel.id })
		if (result === null) return interaction.reply({ content: "Je moet in een ticket zitten om dit command te kunnen uitvoeren.", ephemeral: true })
		const { userId } = result
		var member = interaction.guild.members.cache.get(userId)

		if(!interaction.member.permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) if(interaction.user.id !== member.id) return interaction.reply({ content: "Je hebt geen perms om dit ticket te sluiten.", ephemeral: true })
			// TRANSCRIPTOR
        ...
        if(data) {
            ...
            msgs.forEach(async msg => {
				...
				const embed = new discord.MessageEmbed()
                	...
                	.setFooter("Botz 2021")
					.setTimestamp();
				await member.send({ embeds: [embed], files: [`./${interaction.channel.name}.html`] })
				await fs.unlink(`./${interaction.channel.name}.html`)
					try {
						interaction.channel.updateOverwrite(member.user, {
							VIEW_CHANNEL: false,
							SEND_MESSAGES: false,
							ATTACH_FILES: false,
						...
						return interaction.reply({ content: 'Er is een fout opgetreden, probeer het opnieuw!', ephemeral: true });
					}
	},
};